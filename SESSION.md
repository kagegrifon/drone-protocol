...
## Сессия 22 — 2026-05-13

**Цель:** Исправить проблему, при которой изменение инструкции в программе не влияло на поведение дрона.

**Результаты:**
- Проведён анализ потока данных: мутации `updateInstruction` корректно отражаются через ссылки на массивы в `inlineInstructions` для LOOP-программ, но дрон с flat-программой после завершения уходил в `idle` и больше не запускался
- Добавлен хелпер `resetDroneProgram` в `gameStore.ts`: сбрасывает `callStack`, `state`, `waitingFor` и очищает путь движения для конкретного дрона
- Добавлено действие `restartProgram(droneId)` в store
- Добавлена кнопка **↺ RESTART** в `ProgramEditor` рядом с именем программы — отдельная для каждого выбранного дрона
- Добавлено 3 теста для `restartProgram` в `gameStore.test.ts`
- 113 тестов PASS; TypeScript — 0 новых ошибок

---

## Сессия 21 — 2026-05-13

**Цель:** Исправить блоки «Цель» и Stats — всегда показывали 0.

**Результаты:**
- Найдена корневая причина: `StatisticsSystem.recordOreMined()` никогда не вызывался — только в тестах
- Реализован событийный подход без прямой связанности систем: `StatisticsSystem` подписывается на `ore:dropped` через `gameEvents`
- В `ore:dropped` добавлено поле `amount` (сколько руды сдал дрон на базу)
- Учёт руды засчитывается при сдаче на базу (`drop`), а не при добыче (`mine`)
- `StatisticsSystem` получил метод `destroy()` для отписки при реинициализации игры
- Создан `BACKLOG.md` с описанием незавершённой фичи `congestion` (когда A* возвращает `null`)
