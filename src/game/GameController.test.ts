...
describe("checkWin – ore_mined", () => {
  it("returns false when base ore < target", () => {
    ...
    expect(
      checkWin({ type: "ore_mined", target: 100 }, world, baseId, makeStats()),
    ).toBe(false);
  });
...
describe("checkWin – efficiency", () => {
  it("returns false when efficiency < target", () => {
    ...
    expect(
      checkWin(
        { type: "efficiency", target: 90 },
        world,
        baseId,
        makeStats({ efficiency: 70 }),
      ),
    ).toBe(false);
  });
  ...
  it("returns true when efficiency >= target", () => {
    ...
    expect(
      checkWin(
        { type: "efficiency", target: 80 },
        world,
        baseId,
        makeStats({ efficiency: 80 }),
      ),
    ).toBe(true);
  });
});
