...
describe("stepProgram — WAIT", () => {
  ...
  it("decrements waitRemaining by DT on each tick", () => {
    ...
    expect(
      world.getComponent(id, "Program")!.callStack[0].waitRemaining,
    ).toBeCloseTo(0.2);
  });
...
describe("stepProgram — IF conditions", () => {
  ...
  const numOp = (v: number): import("./types.js").Operand => ({
    kind: "number",
    value: v,
  });
  const fnOp = (
    call: import("./types.js").FunctionCall,
  ): import("./types.js").Operand => ({ kind: "function", call });
  const self = { kind: "self" as const };

  // ── Energy ─────────────────────────────────────────────────────────────
  it("Energy(Self) < 30 is true when energy=20", () => {
    const { id, registry } = addDrone(
      world,
      "running",
      [
        {
    ...
    const { id, registry } = addDrone(world, "running", [
      {
        ...
        conditions: [
          {
            left: { fn: "Deposit", args: [{ kind: "entity", id: depId }] },
            operator: "=",
            right: numOp(0),
          },
        ],
        ...
        conditions: [
          {
            left: {
              fn: "Distance",
              args: [self, { kind: "entity", id: targetId }],
            },
            operator: "<=",
            right: numOp(3),
          },
