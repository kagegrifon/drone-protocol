...
describe("FUNCTIONS — Distance", () => {
  ...
  it("Distance(Self, X) — Self резолвится в droneId", () => {
    ...
    const result = evaluateFunctionCall(
      {
        fn: "Distance",
        args: [{ kind: "self" }, { kind: "entity", id: mine }],
      },
      drone,
      world,
    );
  ...
  it("Distance returns null if any side lacks Position", () => {
    ...
    expect(
      FUNCTIONS.Distance.evaluate([drone, ghost], drone, world),
    ).toBeNull();
  });
});
...
describe("FUNCTIONS — FreeSlots", () => {
  ...
  function makeStation(x: number, y: number) {
    const id = world.createEntity();
    world.addComponent(id, "Position", { x, y });
    world.addComponent(id, "WorkSlots", {
      slots: [{ x, y, occupiedBy: null }],
    });
    return id;
  }
