...
function defaultCallForFn(fn: FunctionName): FunctionCall {
  ...
  switch (fn) {
    case "Energy":
      return { fn: "Energy", args: [self] };
    case "EnergyMax":
      return { fn: "EnergyMax", args: [self] };
    case "Inventory":
      return { fn: "Inventory", args: [self] };
    case "InventoryMax":
      return { fn: "InventoryMax", args: [self] };
    case "Deposit":
      return { fn: "Deposit", args: [self] };
    case "Distance":
      return { fn: "Distance", args: [self, self] };
    case "FreeSlots":
      return { fn: "FreeSlots", args: [self] };
  }
}
      ...
      >
        {FUNCTION_ORDER.map((name) => (
          <option key={name} value={name}>
            {FUNCTIONS[name].icon} {FUNCTIONS[name].label}
          </option>
        ))}
      </select>
